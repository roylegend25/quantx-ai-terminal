...
def _rejection_code(reason,shadow=False,execution_mode="ACTIVE"):
    text=(reason or "").lower()
    if shadow or execution_mode=="SHADOW":return "SHADOW_ONLY"
    if execution_mode=="DISABLED":return "MANUALLY_DISABLED"
...
class ActiveDriveV2Engine:
    ...
    def _candidate(self,db,context,source_type,family,name,version,vote,evidence,shadow=False,eligibility=None,perf=None):
        ...
        if shadow:points=0.
        # Indicator eligibility (see app.decision_engine.eligibility): `shadow`
        # is the pre-existing fixed-placeholder flag (the 8 hardcoded
        # SHADOW_MODELS, always zero-point, unimplemented). `eligibility` is
        # the new per-indicator, per-(symbol,timeframe,mode) status lookup - a
        # REAL indicator moved to SHADOW_ONLY_POOR_PERFORMANCE keeps its real,
        # non-zero points (needed for shadow performance tracking) but is
        # excluded from the ensemble sum via execution_mode, never via a
        # zeroed score (points=0 would be indistinguishable from neutral
        # evidence in the family sum).
        eligibility_status=(eligibility or {}).get("status","ACTIVE")
        execution_mode="SHADOW" if shadow else ("DISABLED" if eligibility_status==STATUS_MANUALLY_DISABLED else "SHADOW" if eligibility_status in SHADOW_STATUSES else "ACTIVE")
        eligible=bool(sign and execution_mode=="ACTIVE"); reason=vote.get("reason") or evidence.get("reason") or "Current-data signal"; p_up=.5+sign*raw/2
        return {"source_type":source_type,"family":family,"source_family":family,"name":name,"source_name":name,"version":version,"source_version":version,"symbol":context["symbol"],"timeframe":context["timeframe"],"direction":direction,
          ...
          "status":"shadow" if execution_mode=="SHADOW" else "disabled" if execution_mode=="DISABLED" else vote.get("status","eligible" if sign else "rejected"),"eligible":eligible,"eligible_now":eligible,"execution_mode":execution_mode,"eligibility_status":"SHADOW_ONLY" if shadow else eligibility_status,"regime_compatible":True,"required_data_available":not("unavailable" in reason.lower()),"rejection_code":None if eligible else _rejection_code(reason,shadow,execution_mode),"rejection_reason":None if eligible else reason,"reason":reason,"evidence":evidence,"data_freshness":context.get("data_status","live")}
    ...
    def evaluate(self,context):
        ...
        settings_scope=context.get("settings_scope","paper"); risk=risk_settings_repository.get_settings(scope=settings_scope,db=db)
        legacy_strategies=list((legacy.get("strategies") or {}).items())
        strategy_vote_list=list(strategy_votes(legacy.get("features") or {}))
        quant_vote_list=list(quant_votes(legacy.get("features") or {}))
        champion=legacy.get("ml_champion") or {}
        # Batch eligibility lookup (see app.decision_engine.eligibility) - one
        # query for every real candidate identity about to be built, instead
        # of one query per candidate alongside the existing per-candidate
        # performance() call.
        identities=[(name,"1.0.0",context["symbol"],context["timeframe"]) for name,_ in legacy_strategies]
        identities+=[(name,"2.1.0",context["symbol"],context["timeframe"]) for name,_,_ in strategy_vote_list]
        ...
        # zeroed contribution would be indistinguishable from neutral
        # evidence (Part 9). Everything downstream that already filters on
        # c["eligible"] inherits this exclusion automatically since `eligible`
        # is defined in _candidate() as execution_mode=="ACTIVE".
        for c in candidates:
            if c["execution_mode"]=="ACTIVE":raw_family[c["family"]]=raw_family.get(c["family"],0.)+c["final_points"]
        ...
        if evidence<risk["min_total_evidence"]:blockers.append("Insufficient total evidence")
        if margin<risk["min_point_margin"]:blockers.append(f"Point margin {margin:.2f} below required {risk['min_point_margin']:.2f}")
        ...
        elif directional is None or directional<risk["min_confidence_to_trade"]:blockers.append("Calibrated directional confidence below threshold")
        ...
        metrics={
          "evidence":{"name":"total_evidence","value":round(evidence,3),"required":risk["min_total_evidence"],"passed":evidence>=risk["min_total_evidence"],"scope":f"{context['symbol']} {context['timeframe']}","formula":"sum(abs(family_points_after_cap))","description":"Total usable signal strength after reliability weighting and family caps.","contributions":[{"family":k,"absolute_points":round(abs(v),4)} for k,v in totals.items()]},
          "point_margin":{"name":"point_margin","value":round(margin,3),"required":risk["min_point_margin"],"passed":margin>=risk["min_point_margin"],"scope":f"{context['symbol']} {context['timeframe']}","formula":"abs(long_points - short_points)","description":"Directional lead after family caps: the minimum absolute difference between the final LONG and SHORT decision scores required before a directional prediction may become actionable.","long_points":round(long_points,3),"short_points":round(short_points,3),"winning_direction":"LONG" if signed>0 else "SHORT" if signed<0 else "NONE"},
          ...
          "confidence":{"name":"directional_confidence","value":round(directional,4) if history_pass and directional is not None else None,"raw_value":round(directional,4) if directional is not None else None,"required":risk["min_confidence_to_trade"],"passed":bool(history_pass and directional is not None and directional>=risk["min_confidence_to_trade"]),"scope":f"{context['symbol']} {context['timeframe']} · decision snapshot","formula":"abs(long_points-short_points)/total_evidence, available only after relevant calibration history","description":"Calibrated directional separation, not a champion-model probability.","failure":confidence_failure},
        ...
        return {"decision_id":None,"engine":self.name.value,"engine_info":{"id":self.name.value,"name":"Active Drive V2","version":self.version,"authoritative":True},"engine_version":self.version,"decision_method":"weighted_ensemble","symbol":context["symbol"],"timeframe":context["timeframe"],"generated_at":now.isoformat(),"expires_at":(now+timedelta(seconds=60)).isoformat(),"performance_snapshot_revision":now.isoformat(),"market_data_revision":legacy.get("data_quality",{}).get("last_candle_time"),
          ...
          "long_points":round(long_points,3),"short_points":round(short_points,3),"neutral_points":0.,"point_margin":round(margin,3),"point_margin_pass":bool(margin>=risk["min_point_margin"]),"total_evidence":round(evidence,3),"evidence_tier":"insufficient_evidence" if not history_pass else "early_evidence","eligible_for_execution":eligible,"blocking_reasons":blockers,"execution":{"eligible":eligible,"risk_gate_passed":False,"reason":blockers[0] if blockers else None},"decision_metrics":metrics,
          "configuration_scope":settings_scope,"configuration_version":risk.get("version"),
          "active_indicators":[c["name"] for c in candidates if c["execution_mode"]=="ACTIVE"],"shadow_indicators":[c["name"] for c in candidates if c["execution_mode"]=="SHADOW"],"disabled_indicators":[c["name"] for c in candidates if c["execution_mode"]=="DISABLED"],"exclusion_reasons":{c["name"]:c["eligibility_status"] for c in candidates if c["execution_mode"]!="ACTIVE"},
