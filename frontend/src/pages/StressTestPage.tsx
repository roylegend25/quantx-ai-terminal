...
function ScenarioDetail({ r }: { r: StressResult }) {
  return (
        ...
        Object.entries(r.checks).map(([component, check]) => (
          <div className="stress-detail-row" key={component}>
            <div className="stress-detail-component">
              {check.ok ? <CheckCircle2 size={13} className="green" /> : <XCircle size={13} className="red" />}
      ...
      <Card title="Scenario Results" full>
        <AutoCardTable
          columns={[
            ...
            { key: "status", label: "Status", render: (r: StressResult) => <StatusBadge status={r.status} /> },
            { key: "reason", label: "Reason", render: (r: StressResult) => r.reason },
            {
              key: "riskResult",
              label: "Risk Result",
              render: (r: StressResult) => (
                <>
                  ...
                  {r.risk_result?.allowed !== undefined && r.risk_result?.allowed !== null && (
                    <div className={r.risk_result.allowed ? "green" : "red"} style={{ display: "block", marginTop: 4 }}>
            ...
            },
            {
              key: "timestamp",
              label: "Timestamp",
              render: (r: StressResult) => (r.timestamp ? new Date(r.timestamp).toLocaleTimeString() : "—"),
            },
          ] as AutoCardColumn<StressResult>[]}
          rows={results}
          keyField={(r) => r.scenario_id}
          titleColumn="scenario"
          statusColumn="status"
          renderDetail={(r) => <ScenarioDetail r={r} />}
          renderActions={(r) => (
            <button
              className="link-btn"
          ...
          emptyMessage='No stress test has been run yet. Click "Run All Scenarios" above.'
        />
      </Card>
    </div>
  );
