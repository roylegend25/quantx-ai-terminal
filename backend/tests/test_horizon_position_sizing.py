...
def test_paper_uses_user_and_authority_not_binance_cap(monkeypatch):
    db=_portfolio(); monkeypatch.setattr(sizing.settings_repository,"get_user_settings",lambda user,db:_risk(500,owner=user))
...
def test_most_restrictive_ceiling_wins(monkeypatch,current,persisted,mode,exchange,expected):
    db=_portfolio(); monkeypatch.setattr(sizing.settings_repository,"get_user_settings",lambda user,db:_risk(current,owner=user))
...
def test_stop_distance_and_exposure_reduce_size(monkeypatch):
    db=_portfolio(10_000); monkeypatch.setattr(sizing.settings_repository,"get_user_settings",lambda user,db:_risk(1000,pct=1,owner=user))
    result=sizing.calculate_position_size(db,user_id="user-a",symbol="BTCUSDT",
...
def test_user_scope_and_missing_policy_fail_closed(monkeypatch):
    ...
    monkeypatch.setattr(sizing.settings_repository,"get_user_settings",
        lambda user,db:_risk(1000 if user=="user-a" else 50,owner=user))
    ...
    monkeypatch.setattr(sizing.settings_repository,"get_user_settings",lambda user,db:(_ for _ in ()).throw(RuntimeError()))
    with pytest.raises(sizing.PositionSizingError,match="RISK_POLICY_UNAVAILABLE"):
...
def test_zero_and_quantity_rounding_fail_closed(monkeypatch):
    db=_portfolio(); monkeypatch.setattr(sizing.settings_repository,"get_user_settings",lambda user,db:_risk(0,owner=user))
